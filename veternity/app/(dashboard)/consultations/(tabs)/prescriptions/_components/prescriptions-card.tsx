...
interface PrescriptionsCardProps {
  ...
  pagePrescriptions: Prescription[];
  page: number;
  totalPages: number;
...
export function PrescriptionsCard({
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-border bg-card shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-border p-4">
        <div className="relative w-full max-w-sm">
          ...
          <Input
            ...
            placeholder="Rechercher un animal, un propriétaire, un médicament..."
      ...
      <PrescriptionsTable prescriptions={pagePrescriptions} />
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-t border-border px-4 py-3">
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <span className="hidden sm:inline">
            {prescriptions.length === 0 ? "0" : `${rangeStart}–${rangeEnd}`} sur {prescriptions.length}
          </span>
        </div>
